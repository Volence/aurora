...
| 15 | **Effects panel polish + a harness evidence gap** — from the overseer's foreground landing pass on item 13 (2026-08-22). All three are cosmetic//evidential; the feature is functionally verified (26/26 CDP rows on the merged tree). (a) **The right-hand panel has no right padding**: in BOTH captured states the `new_scene_id` input, the `Name` field, the `V factor` select and the `Section 0` select run **flush to the window's right edge** (x=1400 of 1400), unlike every other dock in the shell — see `scratchpad/shots-effects-scene/{1-effects-facet-empty,2-scene-two-layers}.png`. Real, consistent, and not flagged by the parcel. (b) **A red `soli…` badge bleeds at the panel's left edge** (~x=1090, y=283) in BOTH shots, including the empty state — so it is almost certainly a pre-existing map-viewport artifact at this window size and **not** introduced by the Effects facet, but **attribution must be checked against another facet before it is assigned to anyone**. (c) **The harness never captures the state the parcel itself tagged for human review.** It flagged "layer cards stack tall with the packed-factor spinners open" — but `LAYERS (2/8)` is **collapsed** in every screenshot, so the density concern is *not judgeable from the captured evidence*. That is an evidence gap, not merely an aesthetic question: the harness should shoot the expanded-layers-with-packed-spinners state, since that is the one nobody has seen. | XS–S | overseer landing pass, item 13 |
