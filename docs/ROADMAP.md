...
| 11 | **Re-point the `parallaxRef` reader to the effects contract** — schema §4's carry (`parallaxRef` → scene id). **GATED on aeon landing its `project.json` edit first**: at aeon `00607dd5`, `project.json:20` still reads `"parallax": ".../ojz_default.asm"`, so re-pointing now would target a key that does not exist. aeon has booked that edit as a wave-1 lane item and an explicit prerequisite (aeon `5d0be056`; empyrean half LANDED at `b0d5b00`). **Contract re-pin due:** the sceneRef parcel pinned empyrean `1326ceb`; the schema doc has since moved to `2f3b6fd` (six→thirteen ref-site relabel + the §4 ordering fix). Verified firsthand that `contract/schema/aurora-effects-scene.schema.json` is **byte-unchanged** across that range and the normative `sceneRef` lines only shifted line numbers — so nothing landed is invalidated — but **re-pin the writer-side golden against `2f3b6fd` when the next parcel is cut** (schema §8). ~~re-pin against `2f3b6fd`~~ — **DONE differently, and better, in item 12**: the schema-codec parcel pins the schema JSON's **git blob hash** (`2d7a9fee37d85334103ca1a3e03e1a40466d6d9c`), which is stable across `1326ceb → 2f3b6fd → 069cf59 → c2c81e2` and cannot be invalidated by a doc-only commit. Cite the blob, not a commit, for anything that depends on the wire shape. **UNGATED 2026-08-22 — aeon landed it. Code anchor `98100905`** (class-checked: a merge, `project.json | 2 +-` = the one line, plus three doc syncs; the narrower edit commit is `7bff8488`). `project.json:20` now reads `"sceneRef": null` — explicit null, not an absent key. aeon verified on the merged tree from a clean checkout, all four build shapes byte-matching their pins, and their reader enumeration came back **DEAD** (four read-only Python readers, all naming fields explicitly, no generic iteration; a full `regenerate-level.sh` re-baked byte-identical, which a consumer of the key could not have done).

⚠ **THE LINE CITES WE HAVE ALL BEEN CARRYING ARE STALE — verified 2026-08-22.** `Act.parallaxRef` is at **`s4-types.ts:239`**, not `:227`; the populate is **`load.ts:396`**, not `:373`. `s4-types.ts:227` today is `export interface Tileset {` — **a different type entirely**, so an agent sent to the booked cite lands in the wrong interface and finds no such field. Still true that ruling 4 targets `Act.parallaxRef` and NOT the dead `Section.parallaxRef` (`:121`, correctly cited, with an explanatory comment at `:129-131`). *A path has a time — and so does a line number.*

**Live state after aeon's landing, checked here:** `load.ts:396` reads `actConfig.parallax`, which **no longer exists**, so `Act.parallaxRef` is now `undefined` while its type says `string | null`. **Harmless today** — `Act.parallaxRef` has **no consumer anywhere in `src/`** (the only other `parallaxRef` hit is `section-ops.ts:29`, which carries the *Section* field, i.e. `cloneSection` copying a dead field — the very literal bar 12 was written about). But it is a typed lie and the re-point closes it. **The save path is SAFE and for the right reason:** `save.ts:254` serialises `config.raw`, the raw parsed document, so `sceneRef: null` round-trips untouched and no `parallax` key is reintroduced — the same never-enumerate discipline as the effects codec, working across a cross-repo key change nobody told it about.

**Fossil noted by the aeon overseer, ruled dead here:** an *untracked* `games/sonic4/data/editor/ojz/act1/export/act_descriptor.asm` carries `dc.l games/sonic4/data/parallax/ojz_default.asm ; act_parallax_config` — the old key propagated into an exported descriptor. It is untracked, syntactically invalid as 68000 asm (a bare path as a `dc.l` operand could never assemble), and **`grep -rn act_parallax_config src/` returns nothing**, so no live Aurora emitter produces it. **But if the export path ever grows an `act_parallax_config` emitter it needs the same re-point** — a second site beyond `Act.parallaxRef` that a search for `parallaxRef` would never surface.

**Cut after item 13 lands** (that parcel is live in `load.ts`/the model; a concurrent re-point would collide). | S | effects arc wave 1 |
