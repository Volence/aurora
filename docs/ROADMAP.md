...
| 17 | **Object labels are drawn untruncated into a fixed 16px box — every map facet, every window size.** Found while ATTRIBUTING the red `soli…` badge in item 15, and left unfixed there on purpose so the attribution stayed clean. `OverlayRenderer.drawObjects` draws the generic no-preview box for a placement that has no sprite preview and centres `obj.typeId` in it in 8px monospace, with no measurement, elision or clip. Measured in the app's own 2D context: `"solid"` is **19.9998px** inside a **16px** box, so it spills ~2px each side *at any size*. **NOT an Effects-facet defect** — the same 16×16 cluster sits at identical canvas coords `[800,202,815,217]` on the Layout facet, from aeon `ojz/act1/section_0.objects.json`'s single `{"x":808,"y":210,"typeId":"solid"}`. What differs is the canvas: **816px wide on Effects vs 876px on Layout**, because the Effects dock is 300px where every other map facet's is 240px — clearance to the canvas's own right edge is **0px vs 60px**. So the badge never moved; the canvas edge did, and this act's one object happens to sit on it. Any `typeId` over ~4 chars spills; any object near a viewport edge shows a half-glyph. Fix is a measured-and-elided label, or a box sized to its text. | XS–S | item-15 parcel, 2026-08-22 |
