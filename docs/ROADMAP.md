...
| 9 | **No aeon-viewport performance datum exists, and the OJZ showcase runs there.** `MapViewport.tsx` has **zero** `requestAnimationFrame` calls (vs. four in `ClassicLevelViewport.tsx`), and `viewStore.ts:52-56` scopes `playAnimatedArt`/`occludeSprites` to `s1` only — so the play-clock + overlay-pass machinery the effects arc was assumed to ride is **classic-only**. **MEASURED 2026-08-22** (`scratchpad/mapviewport-baseline-harness.mjs`, foreground; 21/23 rows): with the page provably still painting — **1602 rAF ticks against 0 MapViewport repaints** over 5.0s idle, probe still bound, React root alive — **the aeon viewport has no clock**; it repaints only on pan/zoom/overlay/state/edit. **Headroom is ample**: worst configuration (zoom 0.25) p95 **1.000ms = 6.0% of a 16.69ms frame**. ⚠ **Two caveats that must travel with those numbers.** (a) `performance.now()` is coarsened to 0.1ms, so only the zoom-0.25 cell is above the noise floor — **cross-cell comparisons are invalid and must not be quoted**; row 3's confound tripwire correctly FAILED and the harness is NOT landed until it passes for the right reason. (b) The bracket excludes React commit, compositor and GPU upload, so these are an **upper** bound on available headroom. **What it does NOT settle:** whether a loop must be ADDED is undecidable from this — nothing in the viewport animates, so "needs none" and "needs one that doesn't exist" predict the identical zero. **PARKED FOR THE OWNER** as a design call; the measurement licenses only the affordability half. **Palette-cycling cost is UNMEASURED** (the stamp stays under `SectionRenderer.ts:13`'s `RECOMPOSE_DIRTY_THRESHOLD = 2000` and takes the per-cell flush path; a palette change takes full-recompose) — booked with aeon as a **wave-2 prerequisite**. | S–M | same |
...
| 11 | **Re-point the `parallaxRef` reader to the effects contract** — schema §4's carry (`parallaxRef` → scene id). **GATED on aeon landing its `project.json` edit first**: at aeon `00607dd5`, `project.json:20` still reads `"parallax": ".../ojz_default.asm"`, so re-pointing now would target a key that does not exist. aeon has booked that edit as a wave-1 lane item and an explicit prerequisite (aeon `5d0be056`; empyrean half LANDED at `b0d5b00`). **Contract re-pin due:** the sceneRef parcel pinned empyrean `1326ceb`; the schema doc has since moved to `2f3b6fd` (six→thirteen ref-site relabel + the §4 ordering fix). Verified firsthand that `contract/schema/aurora-effects-scene.schema.json` is **byte-unchanged** across that range and the normative `sceneRef` lines only shifted line numbers — so nothing landed is invalidated — but **re-pin the writer-side golden against `2f3b6fd` when the next parcel is cut** (schema §8). Note ruling 4 targets **`Act.parallaxRef`** (`s4-types.ts:227`), NOT the dead `Section.parallaxRef` (`:121`). | S | effects arc wave 1 |
