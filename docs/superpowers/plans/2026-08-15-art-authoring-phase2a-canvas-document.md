...
| 8 | With a canvas tab active, press Ctrl+Z after editing a LEVEL | The level does **not** undo. ⚠️ **The mechanism stated here was wrong, and the CDP run proved it.** Reverting the `levelKeysEnabled` canvas branch does *not* move the level: `LevelWorkspace`'s handler calls `focusedHistory()`, the same function `CanvasMode` calls, so under a canvas tab it resolves to the canvas. The ungated handler is a second caller of the canvas's OWN undo. The pool can never move, so this row can never fail. |
| 8b | Draw two separate strokes, then press Ctrl+Z once | Exactly **one** stroke is taken back. This is the real content of the guard row 8 was trying to express — with the branch reverted, the two handlers both fire and a single press consumes both strokes. Added during the CDP run after row 8's plant failed to produce any visible failure. |
...
| 10 | Set the grid origin to a non-zero value (say 3) with the 8px and 16px grids on | The guides **move**, and are otherwise identical to the aligned case — same colours, same weights, and the 16px block grid still reads as distinct from the 8px mesh. The plan's grid model is pure and fully tested, but the two lines of JSX that turn it into strokes are not, and an earlier version of them brightened the 8px mesh and merged it with the 16px one the moment the origin left zero. Screenshot it. |
| 11 | The New Canvas dialog itself | Every field, the live refusal text (type `sky tiles`, then an existing name — read both messages at a normal width; the collision one is long), the disabled Create button, Escape and Enter. Also the polish nobody has seen: is there a focus trap, does clicking the backdrop discard a filled-in form without asking, does Enter submit from the number fields, does an emptied number field show a literal `0`? None of this is verifiable in node. |
| 12 | Create a canvas with **no zone open** | The palette shows the four ramps, not black, and the brush is armed on a visible colour — the status bar should read line 0's brightest entry. A black-looking canvas here means R18's path did not run. |
| 13 | Restart with a canvas as the active tab; then delete its PNG and restart again | First: the tab and its pixels come back. Second: **no toast at boot**, and the tab shows the "could not be loaded" pane with a working Retry. |
| 14 | Force a sidecar rejection (hand-edit the JSON to be invalid), reopen, edit, save | The toast is **legible**: it wraps, stays long enough to read, and the recovery instruction is not clipped. This is the one Task 10 change that could not be falsified in the node suite — there is no DOM there, so the tests assert the CSS is what we intended, not that the message renders readably. Read it on screen at a normal window width. |
...
**R20 (from the CDP run) — what only the running app could show.** 52 checks, 49 passing, over four sessions against real GHZ act 1 data. Full report: `docs/superpowers/plans/2026-08-15-canvas-cdp-report.md`.

**The three defects it found, none of which any node test could have reached:**
1. **A reopened canvas arms an invisible brush.** R18's fix went into `createCanvasDocument` only, so *opening* an existing zone-seeded canvas arms index 1 — black in GHZ's palette. The stroke commits; it just looks like nothing happened. The same defect R18 was raised for, one door over: creation fixed, loading not.
2. **The New Canvas dialog has no focus trap** — Tab walks out of the modal into the app behind it.
3. **An emptied number field shows a literal `0`** (`Number("") === 0`) while the on-screen refusal talks about the *name*, so the message does not describe what is wrong. And **Enter does not submit from the number fields** — only the name input has a handler and there is no `<form>`.

**A claim this run falsified.** Row 8's stated mechanism was wrong, in the plan *and* in `level-keys.ts`'s comment: reverting the guard cannot move the level pool, because both handlers resolve through `focusedHistory()` to the same document. The guard is still load-bearing — without it a single Ctrl+Z consumes two strokes — but for a reason nobody had written down. See row 8b. The lesson generalises: a guard whose *stated* failure mode is impossible may still be protecting against a real one, and only a plant in the running app distinguishes those.

**Verified here and nowhere else:** the offset grid draws every line at exactly its aligned colour (8px `95,95,190`, 16px `130,130,204` at both origins) — and the two-tier plant visibly brightened it, confirming the fix; the sidecar toast renders 560×82px over 5 lines with `scrollWidth == clientWidth`, alive at 8.5s and click-dismissable, with nothing clipped; the exported PNG is read by `file(1)` as "8-bit colormap" and its IHDR parses independently as colour type 3; the project-switch guard raises a real confirm whose Discard leaves the PNG on disk byte-identical.

**A methodology note worth keeping.** Row 13 failed twice, and rather than report it the runner wrote a standalone probe with no Aurora code in it and found Chromium dropping the last `localStorage` writes under a signal-only teardown. A defect that did not exist would otherwise have been reported. Two smaller harness faults were caught the same way. **Suspect the harness before the product.**

**R22 (from the whole-branch review) — three things that were true but never written down.**

1. **An undocumented deferral: there is no resolution UI for a PNG recoloured behind Aurora's back.** `canvas-file-format.ts`'s header says "which file should win is a UI decision for Task 9" — Task 9 shipped without one, and the module picks a winner itself (the sidecar) and warns. That is defensible and the warning names the consequence honestly, but R12 calls this "the single most likely way this design costs a user work", and unlike clipboard, palette-preview and R19 it had no deferral entry. It has one now: **the resolution UI is deferred past 2A**, and the shipped behaviour is sidecar-wins plus an explicit warning.

2. **A departure from the spec's literal wording, and a better design than it.** Spec §4.1 lists "palette-line assignment" as sidecar content. The branch instead puts the line *in the pixel encoding* (`(line << 4) | entry`) and the sidecar carries `{version, profile, palette, gridOrigin}`. That is better — one spelling per colour, no way for pixels and their line assignment to desync — but it is a departure and was never recorded as one.

3. **Latent traps left live:** `canvasStore.setName` has no production caller and `CanvasMode` documents why it must not get one — wired up, it would write to `canvasPngPath(doc.name)` while `source.pngPath` and both mtime baselines still describe the old stem, orphaning the old pair. Either delete it or make it the real rename. Also dead: the `cloneCanvasDoc` re-export from `canvasStore`, and `LoadedCanvas.sidecarRejected` (every consumer reads `source.sidecarRejected`).

**Also flagged, app-wide and deliberate:** the toast `type` now controls dwell as well as severity, so two canvas call sites pick `'error'` for non-fatal diagnostics purely to buy the 10s duration, and 30 pre-existing error toasts across the app went from 2.2s to 10s. Documented rather than accidental, but the right fix is a `'warning'` type or a dwell override so severity and duration stop being one knob.

**R21 (from the CDP re-run) — the fixes hold, and two lessons about verification itself.** Session 2: 16 checks, 16 pass; session 1's 52 re-run against the fixed tree, 52 pass. The reopened-canvas brush now arms a visible colour after a real restart (`42,42,58` → `255,255,255`), and the deliberate non-door holds — two canvases with different palettes, colour 40 picked, `40 → 40 → 40` across both crossings, so the artist's choice survives a tab switch.

**A check that could not fail, found by planting rather than by reading.** Removing the `loadCanvasDoc` arming door failed the restart check as expected — but the *obvious* "close and reopen" check **passed with the bug in place**, because within one session `paintIndex` still holds whatever the create armed. The runner wrote a replacement (reset the index through the guard's Discard, then reopen) *while the plant was still in*, watched it fail, then reverted. This is R11's lesson in a third form: **the intuitive check for a bug is often the one incapable of detecting it.** The non-discriminating check is kept but labelled in both harness and report, because green output from it could otherwise be mistaken for evidence.

**Two harness defects that would have been false reports.** First, "Enter still does not submit" — a bare CDP `keyDown` produces no char event, and Blink's implicit form submission runs off the char event; it had reached the *old* dialog's explicit `onKeyDown`, which is exactly why it appeared to work before and appeared broken after. A correct fix would have been reported as broken. Second, ⌘K intermittently drops one of its three events, surfacing far away as "the form is null". Both were diagnosed by suspecting the harness first — the same discipline that saved the `localStorage` false alarm in session 1. **In UI verification, a failure is evidence about the harness until proven otherwise.**

**R19 (accepted carry-forward) — the `tab-activation.ts` split is deliberately NOT done on this branch.** R17(3) predicted a per-kind extraction would be triggered by a fourth tab kind; R18 recorded that 901 lines had reached the line by growth instead and said to schedule it; Task 13 added 45 more, to 946. It is not being done here, and that is a decision rather than an oversight: extracting four modules out of the file that 27 activation tests and the whole app's tab wiring depend on — at the end of a 14-task branch, immediately before CDP verification — risks real breakage for zero behavioural gain. **Do it at the start of plan 2B, when nothing else is in flight.** The shapes to extract are `tab-activation/{level,sprite,canvas,close}.ts` with the current header staying put as the index.

**R18 (Task 13, from Task 12's review) — a blank canvas is black on black, and Task 13's own contract names the fallback that produces it.** `blankCanvasPalette()` returns 64 zero words — every colour black — and `paintIndex` defaults to `canvasIndex(0, 1)`, which in that palette is also black. So the create-with-no-zone-open path yields **an invisible brush on an invisible surface**, with no in-app signal that anything is wrong. Task 13's contract 3 names `blankCanvasPalette()` as the fallback by name, so this is a live trap, not a hypothetical.

Do **not** fix it by changing `blankCanvasPalette()` — that function is also what pads a short PLTE on load, where "surplus entries are black" is a decision Task 5's review deliberately upheld (a sentinel colour would be worse: there is no "unset" slot in a `CanvasDoc`, so a magenta sentinel becomes indistinguishable from a chosen colour the first time it round-trips). Task 13 needs a *separate* visible default — a simple ramp — used only when creating a canvas outside an open zone, and a paint index that lands on a visible entry of it.

**Also recorded from that review, for whoever picks them up:**
- **Canvas has no copy/cut/paste.** The store's unused `clipboard` field was removed rather than left as surface implying a feature. The `select` tool's marquee-move works; clipboard actions are a deliberate 2A gap and a small follow-up.
- **`SpriteDocHeader` is now the canvas app bar too** — verified store-agnostic, only one tooltip was sprite-specific and is now a prop. The *name* is the remaining lie; `DocHeader` would cost one rename plus a test that pins the symbol.
- **`tab-activation.ts` reached 901 lines.** R17(3) predicted the split would be triggered by a fourth tab kind; it has instead been reached by growth within the third. Schedule the per-kind extraction rather than waiting on `palette-doc`.

**R17 (Tasks 12 and 13, from Task 11's review) — three things this branch has deliberately deferred, recorded so they are decided rather than discovered.**

1. **Session restore does not reach canvas tabs at all (Task 13 owns this).** `session-lifecycle.ts`'s `isValid` accepts only the project-setup tab, enumerated level tabs and `parseSpriteDocTabId` matches, so `pruneSession` deletes a restored `doc:canvas:` tab before anything can activate it, and the restore effect hand-dispatches activation for sprite and level ids only. **R14's own justification for making `activateCanvasDoc` total — "session restore genuinely reaches this" — is therefore false in the tree today.** The guard is still right, for a different reason: totality is what makes "clear the canvas focus" expressible, since handing a non-canvas tab id to a total function is how that is spelled. The mechanical fix is two lines and is safe (the project dir resolves from the same store pair the restore effect derives its own key from), but the *policy* is not obvious: a canvas whose PNG was deleted between sessions would toast for 10 seconds at every boot, forever, on a tab that may not even be focusable to close (see 2).

2. **A canvas whose file cannot be read is currently unfocusable, and Task 12 must decide whether that is right.** `requestOpenTab` gates `session.open` on the load, so a failed load toasts and the tab never takes focus. The sprite precedent argues the other way: `runSpriteActivation` returns true for its `'deferred'` case precisely so the tab keeps its place and renders an inert pane. And unlike `'deferred'`, a missing file is *permanent* — the user gets a fresh toast on every click of a tab they can never select. Opening onto an honest "could not load" pane is probably better; Task 12 owns the call.

3. **`tab-activation.ts` is now 811 lines** and is the last addition that can be justified as one coherent subject (three parallel activation systems plus the close path, readable as variations on one shape). `TAB_KINDS` already lists a fourth kind, `palette-doc`. When that arrives, extract per-kind modules and keep the header here as the index.

**R16 (CRITICAL, Task 10 — and a gap this plan never covered) — the project-open guard did not know canvases exist.** Task 10 correctly made `resetProjectRuntime` close every canvas document (a document that outlives its project holds an absolute path into the old one, so a later Ctrl+S writes across projects). But `planProjectOpen` in `src/renderer/shell/project-open-guard.ts` weighs only `classicDirty`, `aeonDirty` and `spriteDirty` — so a dirty canvas was destroyed on a project switch with no dialog and no toast. **This plan does not mention `project-open-guard` anywhere in its task text; the gap had no owner until Task 10's review found it**, which is the argument for reviewing the surrounding system rather than only the diff.

Two details worth keeping: the guard's own header says it exists because "openPath previously reset stores with no confirm — silent data loss", and its inline comment says sharing the tab-dot predicate is what keeps it "from being narrower than what the strip shows". Canvas reintroduced exactly that divergence. Also, the guard's *save* branch routes through `saveAllDirty`, which only writes **saveable** canvases — so a destination-less dirty canvas correctly aborts the open, but needs its own copy, because telling a user to "save or discard" a canvas they cannot save is a dead end.

**Still owed, and assigned here so they are not rediscovered:**
- **Task 11** must consume `LoadedCanvas.warnings`. R12 exists because `decodeCanvasFiles` computed careful diagnostics and dropped them; they are now computed one layer higher and dropped there instead. The load-time warning is an artist's *first* notice that a sidecar was rejected — without it their first signal is a save-time toast.
- **Task 11** must add `confirmCloseCanvasDoc`. Until it exists, the unsaved dot on a canvas tab has no close confirmation behind it.
- **Task 13** must guarantee a canvas has a destination at creation, or route Ctrl+S on a source-less canvas to the naming dialog. The dot deliberately covers unsaveable work (a dot that answers "is there unsaved work" is not the same question as "can I write it"), but a dotted tab where Ctrl+S is silently inert, with no close prompt and no explanation, is a dead end. The sprite precedent has an escape — Export — and canvas currently has none.

**R15 (IMPORTANT, Tasks 9 and 10) — a save result has to be able to tell the truth, and "clean" must mean "saved".** Task 9's review found the module correct and its edges under-defended, in a way Task 10 would have inherited:

- **The path guard was enforced but unpinned.** Deleting *both* `canvasNameIsSafe` call sites left all 2537 tests green. This matters more than the usual instance of that failure: `readBinaryFile` in the main process deliberately has no `isRelPathSafe` (adding one would break aeon's absolute-path chunk import — see `classic-file-access.ts`'s header), so the renderer-side check is the only guard on the read path.
- **R12's fix moved its own failure one step downstream.** After a rejected sidecar, `sidecarRejected` can only clear by reloading, which re-rejects — so `profileId` and `gridOrigin` (the only sidecar-exclusive fields; the palette round-trips through PLTE) are dropped on *every* save for that document's life. The user was told once at load and nothing at save: `{ ok: true }`, dirty dot cleared, half the document not persisted. `SaveCanvasResult` now carries `sidecarWritten` so Task 10 can say it.
- **A deliberate omission must not destroy the mtime baseline.** Returning `null` for the skipped sidecar reads as "did not exist at read" per `save-guard.ts`, so the next real sidecar save would conflict with a misleading "appeared externally". The expected baseline is carried through instead.
- **Partial writes and channel throws need their own shapes**, as `classic-save.ts` already models — otherwise a PNG that landed leaves Task 10 holding a stale baseline and the next save conflicts on a file Aurora itself wrote.
- **`sidecarRejected` lives on `CanvasSource`.** Returned as a bare sibling it had no home in the store, and a flag with no home is a flag Task 10 drops — reopening R12 one level up.
- **A locked sidecar is the same chain through another door:** a bare `catch` treated EACCES as absence, silently degrading to "plain Aseprite export" and overwriting on the next save.

**Left for Task 10 to decide:** nothing clears `CanvasSource.sidecarRejected` after a save, so a document that started with an unreadable sidecar keeps dropping its profile and grid origin for the rest of the session. Recovery does exist — fix the JSON by hand and reopen, and it parses — but the user has to be *told* that, so Task 10's toast on `sidecarWritten: false` must say what happened and what to do about it, not merely that something was skipped. An in-app "overwrite it anyway" is the fuller answer and is deliberately deferred past 2A.

**R14 (IMPORTANT, Tasks 7 and 11) — the store's implicit contracts, written down before five consumers bind to them.** Task 7's review found the code sound and the *contracts* unstated. Settled:

- **`setPixels` throws on a dimension mismatch.** The realistic caller is not a resize feature — it is a Task 12 paint handler whose dep array misses `docId`, built while an 8×8 document was active and firing after a switch to a 16×16 one. Silently accepting that destroys the second document's art *while pushing an undo entry and a dirty dot that make it look deliberate*. A silent no-op would be worse still, turning visible corruption into an invisible dropped edit. A dimension mismatch is programmer error, so `throw` is the right channel whatever the setter returns. Resize, when it exists, gets its own action so intent lives at the call site.
- **`activeDocId` is `string | null`, and `activateCanvasDoc` is total.** Activating a not-yet-loaded document used to return silently, leaving `activeDocId` on the *previously focused* canvas — the pane showing document X under tab Y, which is exactly what `closeCanvasDoc` was written to prevent. Canvas tab ids are stable across sessions, so session restore reaches this: the tab exists before the file has been read.
- **`gridOrigin` is an EDIT** — setter records, and it joins `CanvasSnapshot`. The argument is stronger than "same as `profileId`": the origin decides where the tile grid starts, therefore *which pixels share an 8×8 cell*, therefore which per-cell palette-line clashes 2B will report. It is materially the same category as the profile, not an analogy to it. (The case against — grid lines are guides, and `visibleGrids` is view state — fails on the fact that `visibleGrids` is not persisted and this is.) It was persisted and round-tripped with no way to change it, so Task 12's only route would have been mutating through the exposed `docs` map, bypassing both undo and the dirty flag.
- **Ownership and aliasing are stated, not implied.** `setPixels` takes ownership of the buffer it is handed (cloning 1 MB per stroke is not the answer); `readCanvasSnapshot` returns live references and is history-plumbing, with `canvasDocState` as the consumer-facing read.

**What Task 11's activation glue must guarantee**, given the above: (a) load then activate, never activate then load; (b) activation fires on *every* tab focus change, including the one `session.closeTab` performs after a close, and clears on a switch to a non-canvas tab; (c) the pane derives its document id from the **active tab id** and treats `activeDocId` as a mirror to validate against, not an independent source of truth. If (c) holds, the whole stale-pane class disappears.
