...
- **`gridOrigin` is an EDIT** — setter records, and it joins `CanvasSnapshot`. Same reasoning R13 applied to `profileId`. It was persisted and round-tripped with no way to change it, so Task 12's only route would have been mutating through the exposed `docs` map, bypassing both undo and the dirty flag.
