...
*2026-08-30, later still: the FIFTH expiry fired at aeon `4a4d3474` ("step 6 measured: the section-5 authored band IS SEEN on screen - picture, control, A/B CRAM tables", ancestor of their `origin/master` `e6405428`), which committed `docs/research/reference_captures/2026-08-30-sec5-band/` — README, two byte-identical bound runs (CRAM line 2 entry 8 `$0EA4` at lines 40/56/72, `$0000` at 8/20/96/150, one frame) and a control (`$0000` everywhere, `Raster_Program` 0), on their headless `oracle-aether`. The "no ROM was built, no emulator was run, no CRAM was sampled" line above is still true of THIS packet; the measurement is theirs. `RASTER_SECTION_BINDING_LIMIT` now cites that artifact in place of `c9a462be`'s "nothing has been seen on screen", retired against `e6405428` on branch `parcel/o63-band-seen-expiry` (constant at aurora `ada54128`, tests at `b780fafa`). The packet above is left as written.*
