...
| id | why left |
|---|---|
| ⚠ **C6 and C10 below were TAKEN — this table was not updated and it generated a phantom queue row on 2026-09-10** | Both say *"Left, and it should be taken"*, and both were, four days before that row was written: **C6 at `9cd3ed9a`** (the over-warning rewritten as the rule, and locked by `guides.test.ts` asserting the old sentence cannot come back) and **C10 at `434fd80c`** (the re-bake is a step of the path now, not a recovery), with `b6180806` refuting two more §6 sentences on the same derivation. A disposition table is a snapshot of a decision, and this one kept reading as a backlog. **The row it produced was re-measured before any work started and closed the same minute** — but only because the re-measure happened; the table itself said take them. ⚠ **C10 carries NO test**, unlike C6, so its correction can silently regress. |
