...
**SWEPT SUITE-WIDE 2026-09-18, so nobody re-runs it: this doc was the ONLY over-broad one.** I guessed five lanes carried the shape and said so as a worry; the hub measured each lane at its committed tip and found **one** — this file. ⚠ **THE GOVERNING TEXT IS PROTOCOL BAR 10 AND IT WAS IN EVERY LANE'S BOOT READ THE WHOLE TIME** — its live instance already says *"the hazard is `mcp__oracle__*`, which blocks with no timeout and no interrupt path — **not** 'an emulator ran'. Headless subprocess emulators run unattended nightly in aeon and are fine."* **Cite bar 10, not this paragraph and not a lane's doc.** Oracle's file restates it correctly — *"aeon's binding constraint is that their subagents cannot use MCP"* — **re-derived here firsthand rather than taken from the relay, at oracle `origin/main` `5b3d551`, `docs/2026-08-22-aeon-instrument-asks.md` line 83** (`git -C ../oracle show origin/main:docs/2026-08-22-aeon-instrument-asks.md`). Aeon had generalised from it to "it boots an emulator, and agents cannot", which is aeon's to settle.

⚠ **THE WRITING-TIME CHECK, the hub's, 2026-09-18 — and it is the only one anybody has that fires before review.** This defect is protocol bar 10 (*a ban stated wider than its justification*; **read it there, it is contract text and not this lane's observation** — and note the phrase wraps across a line, so a grep for it can come back empty and read as an absence). **Three over-wide rules were found in one night — this file's, the dispatch skill's, and the hub's own contract text — and ALL THREE carried their justification ADJACENT, in the same sentence:** *"at least one glyph on the glass (an empty text shape is not a run)"*, *"No emulator, ever. Never touch `mcp__oracle__*`"*, *"runs the foreground harnesses (agents cannot)"*. In every case the reason was already written down and the rule above it was simply bigger.
**So the check is NOT 'derive the justification' — it is already there. It is: read the parenthetical you just wrote, and ask whether the rule above it is wider.** Cheap enough to run while writing, and it would have caught all three. ⚠ **It does not reach a rule whose justification was never written down, and nobody has a mechanism for that one** — say so rather than treating this as complete coverage.
⚠ **AND THE HARDER HOLE, recorded OPEN because nobody has anything for it: bar 10 was present, true, correct and READ, and it still did not fire when three lanes hit its exact subject in one night.** I read it at boot and re-derived it as my own observation hours later; the hub cited it to aeon and then banked a duplicate of it an hour after that; aeon reconstructed its scoping from a governance date, a timer result and a source header rather than from the bar. **Seeing a duplicate and creating one are not in tension when the second act feels like consolidation.** So neither reading the rule nor citing it is sufficient — the gap is between holding a rule and connecting it to the case in front of you, and the writing-time check above does nothing for it. **Do not close this with a bar that would not have worked.**
⚠ **AND IT HAS A THIRD SHAPE, WORSE THAN THE OTHER TWO BECAUSE THE RULE IS BEING APPLIED CORRECTLY WHILE IT HAPPENS (2026-09-18, my own `scripts/failure-class-reporter.mjs`).** That file refuses to let a non-run read as a pass — it names every skipped test with its reason, which is exactly the discipline — **and makes that distinction for SKIPS and not for TRUNCATION, in the same file, by the same author.** Nothing was forgotten and no rule was unread: the principle was learned attached to a CASE rather than to a SHAPE, so it hardened one surface and left its sibling open. **The question that finds these, and it is the only one I have: what is the SIBLING of the thing I just protected?** Ask it of the surface, not of the rule — a skip and a truncated run are the same shape (*a thing that did not happen reading as a thing that passed*) and look nothing alike.
