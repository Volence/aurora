...
⚠ **TWO WAYS A LANDING'S VERDICT REACHES YOU WRONG** *(oracle's, found against its own landings 2026-09-12; relayed by the hub, anchor `oracle origin/main 760f832` verified reachable here before this line was written. Aurora had a live dependency — two parcels landing that hour — which is why it was relayed and why it is banked.)*

**(1) THE HARNESS'S REPORTED EXIT CODE IS NOT THIS SCRIPT'S VERDICT**, and the two disagreed twice in one day over there: an `&` nested inside a backgrounded call tracked the OUTER shell and reported 0 while the log stopped mid-run, and a chained command reported the trailing `grep`'s status, announcing a RED landing as *exit code 0*. **Read the log's own verdict line** — `land: origin/master <old> to <new>, and it is the tree the suite ran on`, or `land: REFUSED, …` — never the exit code. After a pipe `$?` is the LAST element's status, and this shell has no `PIPESTATUS`.
**Checked here rather than assumed, 2026-09-12, because this lane lands through `npm run land 2>&1 | grep -E "^land:…"`:** the pattern does match `land: REFUSED` (piped through it as a test), and the script dies at `:129` writing that line to stderr, which `2>&1` captures — **so the refusal would have surfaced. That is a property of the pattern, not protection.** A `tail -n` after it can still cut the line off. **The independent check that owes nothing to the script's own output is `git ls-remote origin refs/heads/master` against `git rev-parse master`** — eight SHAs reported landed this session, all confirmed present on the remote ref that way.
**(2) DO NOT EDIT THE TREE WHILE A LANDING RUNS — NOT EVEN DOCS.** Oracle edited two docs mid-run and a gate correctly reddened an otherwise-green landing, because **one of those files is compiled into a test**, so the run could no longer prove it had tested the committed bytes. The rule is the suite-run-and-push-as-one-act rule seen from the other side: the script guards HEAD moving and a dirty tree at its edges, and a mid-run write lands inside them. **Agents in worktrees are not this hazard** (separate trees); the overseer's own keyboard is. **`docs/lane-status.json` is the tempting exception here and is genuinely safe** — gitignored, and the bounds gate that reads it drives FIXTURES rather than the live path — but that is a fact to re-check if either changes, never a habit to generalise to other docs.
...
- **⚠ THAT SAME `node_modules` SYMLINK BREAKS `git check-ignore` IN A WORKTREE, AND `check-cited-paths` DIES `COULD NOT MEASURE` RATHER THAN PASSING OR FAILING** *(2026-09-12, found by the ramp-rig agent; the sibling of the row above, same cause, different victim)*. `git check-ignore` refuses to answer for a path **beyond a symbolic link**, so the gate cannot classify anything under `node_modules` and stops without a verdict. **`cp -al` over the symlink fixes it.** Worth the line because the failure is **loud and correct** — it reports that it could not look, rather than rendering not-looking as a clean result — so an agent meeting it should fix the tree, never route around the gate. **Put it in the dispatch brief**: every agent worktree hits it, and an agent that has not been told reads a gate it has never seen fail as a repo fault.
  ⚠ **`ELECTRON_BIN` ALONE SILENTLY MEASURES THE MAIN CHECKOUT'S BUILD — IT NAMES ONE FILE, NOT A TREE** *(2026-09-06, `docs/reviews/2026-09-06-rowremap-three-red.md`)*. A worktree with no `node_modules` fails `isRunnableTree` (`scratchpad/lib/run-root.mjs:134`), so the walk falls through and the harness drives **main aurora's `dist/`** while every path in the output looks right. An agent ran three passes this way **including its baseline**, and it surfaced only because a planted mutation printed **PASS against an unmutated app**. **`AURORA_BUILT_TREE=<the worktree>` is the missing half** and has **no default by design** (`run-root.mjs:217`), so it must be passed explicitly. Put BOTH in every dispatch brief that runs a harness from a worktree. The run announces which tree answered on its `root:` line and its `pinned:` line (`in-tree:` instead, only when the tree has its own electron binary; corrected 2026-09-12 by the scene-selection parcel) — **read them**, they are the only signal.
