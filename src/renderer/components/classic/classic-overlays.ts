...
/**
 * Clear world-space room kept between a marker border's inner edge and glyph
 * ink, in SCREEN pixels (the caller multiplies by `invZoom`). Both marker
 * borders here are drawn at `ctx.lineWidth = 1 * invZoom`, i.e. one screen
 * pixel, so their budgets are screen-space quantities too. See
 * `canvas/label-fit.ts` for what the budget is spent on.
 */
export const MARKER_LABEL_GAP_PX = 0.5;
/** `ctx.lineWidth` these markers are stroked at, in screen px. */
  ...
  d.objects.forEach((obj, i) => {
    ...
    if (obj.id === RING_OBJ_ID) {
      ...
      const gfit = fitLabelInContext(ctx, s1ObjectName(obj.id),
        labelBudget(gw, MARKER_STROKE_PX * invZoom, MARKER_LABEL_GAP_PX * invZoom));
      ...
      const hfit = fitLabelInContext(ctx, obj.id.toString(16).toUpperCase().padStart(2, '0'),
        labelBudget(HEX_MARKER_SIZE, MARKER_STROKE_PX * invZoom, MARKER_LABEL_GAP_PX * invZoom));
