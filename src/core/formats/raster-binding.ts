...
export const RASTER_SECTION_BINDING_LIMIT =
  ...
  + '(2026-09-17): tools/effects_gen.py resolves a rasterRef against the preset documents, refusing '
  ...
  + 'editor\'s writer (aeon c9a462be). Leaving a wired owner unbound resolves to that hand: label '
  ...
  + 'not SILENT: aeon\'s tools/effects_seam_gate.py refuses the build for it and names the owner and '
  + 'the id ("no preset threads ojz_act1_preset_raster(preset: <record>_KEY)"). The refusal is '
  ...
  + 'pytest lane sits under NO_LINT, and FAST=1 builds the tree. On a section-mode act NOTHING HERE '
  ...
  + 'call-site line in that record\'s preset(). Nor is there anything to look at here: binding a '
  + 'raster preset draws nothing, because the viewport does not composite a rasterRef, so unlike '
  + 'assign_section_bg (whose ref the viewport does composite) this assignment changes nothing on '
  + 'screen. A preset document costs ROM whether or not anything binds it, since aeon emits one '
  + 'program per document. Unlike assign_section_scene, which is baked. The section-5 band has been '
  ...
  + 'saying what is quoted here, or when this viewport learns to composite a rasterRef (owner: aeon\'s '
