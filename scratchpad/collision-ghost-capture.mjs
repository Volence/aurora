...
async function main() {
  ...
  try {
    ...
    // ---- THE PAINT GHOST: what the author sees BEFORE committing ---------
    //
    // Owner complaint 4 is about PAINT TIME specifically ("when I go to paint
    // you can't really see what the shape looks like over the art"), so the
    // ghost is its own capture rather than something inferred from the picker.
    // It needs three things at once, and none of them is a screenshot concern:
    // the paint-collision tool active, a NON-AIR shape selected (air ghosts as
    // an erase tint with no shape at all), and a live hover cell — the ghost is
    // driven by `previewHoverRef`, which only a real mousemove sets.
    {
      // Pick an angled shape: the first thumbnail whose degree label is a real
      // number, not the '—' the no-angle shapes carry.
      const picked = await c.evalExpr(String.raw`
        (() => {
          const btns = [...document.querySelectorAll('button')].filter(
            (b) => /^-?\d+°$/.test((b.textContent || '').trim().split(/\s+/).pop() || ''));
          // Prefer a steep one so the mark's direction is unmistakable.
          const withDeg = btns.map((b) => ({
            b, deg: parseInt((b.textContent || '').trim().match(/(-?\d+)°/)[1], 10),
          })).filter((e) => Math.abs(((e.deg + 180) % 360) - 180) >= 25);
          const target = (withDeg[Math.floor(withDeg.length / 2)] || { b: btns[btns.length - 1] });
          if (!target.b) return 'no-angled-thumbnail';
          target.b.scrollIntoView({ block: 'center' });
          target.b.click();
          return 'picked ' + (target.deg ?? '?') + '°';
        })()`);
      log('  picker:', picked);
      await sleep(500);

      // ARM THE TOOL. Picking a thumbnail does NOT arm it — measured: the tool
      // stayed 'view' and the ghost is gated on 'paint-collision', so the first
      // run of this capture produced a shot with no ghost in it at all. The
      // shortcut is 'c' (workspace/tool-meta.ts). Try the toolbar button first,
      // fall back to the key, and READ THE TOOL BACK either way.
      const toolBtn = await c.evalExpr(clickByText('/Paint Collision/'));
      log('  clicked Paint Collision button:', toolBtn);
      await sleep(300);
      let tool = await c.evalExpr('window.__dbg.aeon.state().tool');
      if (tool !== 'paint-collision') {
        for (const type of ['keyDown', 'keyUp']) {
          await c.send('Input.dispatchKeyEvent', { type, key: 'c', code: 'KeyC', text: type === 'keyDown' ? 'c' : undefined, windowsVirtualKeyCode: 67 });
        }
        await sleep(400);
        tool = await c.evalExpr('window.__dbg.aeon.state().tool');
      }
      log('  tool now:', tool);
      env_report.ghostTool = tool;
      if (tool !== 'paint-collision') {
        log('  !! TOOL NOT ARMED — the ghost cannot draw; the shot below would be misleading.');
      }

      // Park where the curve is, then hover a cell ON the curve.
      await c.evalExpr(`window.__dbg.setView(${PARK[8][0]}, ${PARK[8][1]}, 8)`);
      await sleep(900);
      const rect3 = await c.json(CANVAS_RECT);
      const view3 = await c.json('window.__dbg.view()');
      // AIM AT AN INTEGER CLIENT PIXEL. World (1200, 556) is on the slope; the
      // client pixel is derived from the app's own transform and then ROUNDED,
      // because CDP delivers integers and a fractional ask lands one row off.
      const wx = 1200, wy = 556;
      const cx = Math.round(rect3.left + (wx - view3.x) * view3.zoom);
      const cy = Math.round(rect3.top + (wy - view3.y) * view3.zoom);
      log(`  hover aim: world (${wx},${wy}) -> client (${cx},${cy})  [rect.left=${rect3.left} rect.top=${rect3.top} view=${JSON.stringify(view3)}]`);
      env_report.ghostAim = { world: { wx, wy }, client: { cx, cy }, rect: rect3, view: view3 };
      for (const y of [cy - 2, cy - 1, cy]) {
        await c.send('Input.dispatchMouseEvent', {
          type: 'mouseMoved', x: cx, y, buttons: 0,
        });
        await sleep(160);
      }
      await sleep(700);
      shots.push(await shot(c, 'after-paint-ghost-z8'));
    }
