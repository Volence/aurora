...
//
// `New tile animation` is a creation form and was the tallest box in the
// effects column, so ROADMAP item 41 gave it `defaultCollapsed`; `Tile
// animations` joined it with item 45's open tail. A collapsed
// CollapsibleSection renders NO children, so every control below would come
// back `null` and read as "the control is missing", which is the defect this
  ...
  try {
    ...
    const headings = await c.json(
      `[...document.querySelectorAll('span')].map(e => (e.textContent||'').trim())
        .filter(t => /^(Tile animations|New tile animation$)/.test(t))`);
    ...
    check('4a', 'a rate_shift control exists in the New tile animation form', ui.select !== null,
