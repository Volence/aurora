...
// ── discovery-file and process hazards (see the header) ────────────────────
const DISCOVERY_FILES = ['.aurora', '.sonic-level-editor'].map((s) => join(homedir(), s, 'mcp.json'));
...
function descendants(root) {
  ...
  while (grew) {
    grew = false;
    for (const [pid, ppid] of parent) if (!out.has(pid) && out.has(ppid)) { out.add(pid); grew = true; }
  }
  return out;
}
