#!/usr/bin/env node
// BEFORE-state screenshots of aurora's aeon collision overlay.
//
...
async function main() {
  ...
  try {
    ...
    env_report.shots['before-slope-z4'] = await park(4);
    shots.push(await shot(c, 'before-slope-z4'));

    env_report.shots['before-slope-z8'] = await park(8);
    shots.push(await shot(c, 'before-slope-z8'));

    env_report.shots['before-slope-z1'] = await park(1);
    shots.push(await shot(c, 'before-slope-z1'));

    // ---- control: angles OFF, same z4 centre -----------------------------
    env_report.shots['before-noangles-z4'] = await park(4);
    ...
    shots.push(await shot(c, 'before-noangles-z4'));
    ...
    await sleep(1100);
    env_report.shots['before-slope-z4-collision-facet'] = {
      asked: { vpX: vpX2, vpY: vpY2, zoom: 4 },
      actual: await c.json('window.__dbg.view()'),
    };
    shots.push(await shot(c, 'before-slope-z4-collision-facet'));
    ...
    if (panel) {
      shots.push(await shotClip(c, 'before-picker', {
      ...
      shots.push(await shot(c, 'before-picker'));
    }
