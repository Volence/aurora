...
const child=spawnGuarded('/usr/bin/xvfb-run',['-a','-s','-screen 0 1680x1050x24',ELECTRON,`${ROOT}/dist/main/index.mjs`],{cwd:ROOT,env,stdio:['ignore','pipe','pipe'],detached:true});
...
try{
  ...
  // probe-zoom.mjs's stepper only doubles/halves from 2, so it can never SIT on
  // the store's default zoom of 24 — targeting 24 lands on 32 and silently
  // re-measures the 32 row. 24 is the zoom a user actually opens the tile tier
  // at (192px canvas centred in a 240px box, K = 24), and the row with the
  // largest predicted correction, so this variant measures it by not stepping
  // at all: `startZoom === null` means "whatever the app opened with".
  const trial = async (startZoom) => {
    const z = startZoom === null ? await c.ev('window.__h.zoom()') : await setZoom(startZoom);
