...
//       and its inline font-size IS that declaration (not `inherit`). Token
//       from primitives.tsx (lib/chip-declared-size.mjs), px from the live
//       document. A copy of chunk-links-harness.mjs row 4b; answered by the
//       owner's d-36 rule (docs/reviews/2026-09-13-chip-font-majority.md).
...
async function main() {
  ...
  try {
    ...
    //
    // CHIP-FONT-13PX, answered by the owner's d-36 rule: `Chip` DECLARES
    // T.tBase for both branches (docs/reviews/2026-09-13-chip-font-majority.md).
    // This is a copy of chunk-links-harness.mjs row 4b and follows it.
    //
    // THE EXPECTATION IS DERIVED, NEVER TYPED, AND IT FOLLOWS THE PRIMITIVE:
    // the token from primitives.tsx (lib/chip-declared-size.mjs, the tree this
    // run BUILT), its px from the live document, and the inline font-size
    // must read back as that token's var(), which is what catches a `font`
    // shorthand erasing it when the declared and inherited sizes are equal.
    //
    ...
    // element mounted right now, with its computed and inherited size.
...
async function runPropagationRows(c, ctx) {
  ...
  // ── PROBE ONLY: the document cell and the library chunk, BEFORE the stamp ──
  const probeBefore = await c.json(`(() => ({
    cell: window.__dbg.aeon.artDocCellAt(${cell.cx}, ${cell.cy}),
    chunk: window.__dbg.aeon.chunkInfo(${JSON.stringify(chunkId)}),
  }))()`);
  console.log(`\n  PROBE before stamp: ${JSON.stringify(probeBefore)}`);

  // Stamp the cell on the REAL composer canvas — SPLIT, so the document can be
  // read BETWEEN the down (which stamps) and the up (which commits).
  const probeCell = `(() => {
    const h = window.__dbg.aeon, o = h.artChunkOpen();
    const cells = (o.widthTiles >> 1) * (o.heightTiles >> 1);
    let a = 0, b = 0, an = 0, bn = 0;
    for (let i = 0; i < cells; i++) {
      const wa = h.artDocCollisionAt('a', i), wb = h.artDocCollisionAt('b', i);
      a += wa; b += wb; if (wa) an++; if (wb) bn++;
    }
    return {
      cell: h.artDocCellAt(${cell.cx}, ${cell.cy}),
      chunk: h.chunkInfo(${JSON.stringify(chunkId)}),
      dirty: o.dirty, tool: o.tool,
      collA: { sum: a, nonzero: an }, collB: { sum: b, nonzero: bn },
    };
  })()`;
  ...
  const probeImmediate = await c.json(`(() => ({
    cell: window.__dbg.aeon.artDocCellAt(${cell.cx}, ${cell.cy}),
    chunk: window.__dbg.aeon.chunkInfo(${JSON.stringify(chunkId)}),
    open: window.__dbg.aeon.artChunkOpen(),
    toasts: window.__dbg.toasts(),
  }))()`);
  console.log(`  PROBE 0ms after click: ${JSON.stringify(probeImmediate)}`);
  const probeScan = await c.json(`(() => {
    const h = window.__dbg.aeon, o = h.artChunkOpen(), hits = [];
    for (let y = 0; y < o.heightTiles; y++) for (let x = 0; x < o.widthTiles; x++) {
      const cl = h.artDocCellAt(x, y);
      if (cl && cl.atlasTile === ${armed}) hits.push([x, y]);
    }
    const cv = [...document.querySelectorAll('canvas')].find((k) =>
      k.parentElement && k.parentElement.style.margin === 'auto'
      && k.parentElement.style.padding === '24px' && k.offsetParent !== null);
    const r = cv.getBoundingClientRect();
    return { hits, cssW: r.width, cssH: r.height, backW: cv.width, backH: cv.height,
             left: r.left, top: r.top, dpr: window.devicePixelRatio };
  })()`);
  console.log(`  PROBE doc cells now holding the armed tile: ${JSON.stringify(probeScan)}`);
  await sleep(400);
  const probeAfterStamp = await c.json(`(() => ({
    cell: window.__dbg.aeon.artDocCellAt(${cell.cx}, ${cell.cy}),
    chunk: window.__dbg.aeon.chunkInfo(${JSON.stringify(chunkId)}),
  }))()`);
  console.log(`  PROBE after stamp:  ${JSON.stringify(probeAfterStamp)}`);
  ...
  // ── PROBE ONLY (chunk-row9-probe.mjs): which HALF of the save failed ──
  // `dirty` after the click separates "saveComposerDocument returned early"
  // (an early return leaves the document dirty, because only the successful
  // tail re-opens it clean) from "it ran and the propagation was vacuous".
  // The toasts carry the refusal sentence when it is the first.
  const probeAfterSave = await c.json(`(() => ({
    cell: window.__dbg.aeon.artDocCellAt(${cell.cx}, ${cell.cy}),
    chunk: window.__dbg.aeon.chunkInfo(${JSON.stringify(chunkId)}),
  }))()`);
  console.log(`\n  PROBE after save:   ${JSON.stringify(probeAfterSave)}`);
  const probeOpen = await c.json('window.__dbg.aeon.artChunkOpen()');
  const probeToasts = await c.json('window.__dbg.toasts ? window.__dbg.toasts() : '
    + '(window.__dbg.aeon.toasts ? window.__dbg.aeon.toasts() : "no-toast-hook")');
  console.log(`\n  PROBE after save: open=${JSON.stringify(probeOpen)}`);
  console.log(`  PROBE toasts: ${JSON.stringify(probeToasts)}`);
  const probeDoc = await c.json(`(() => {
    const h = window.__dbg.aeon;
    return h.chunkPlacements ? h.chunkPlacements(0).length : 'no-hook';
  })()`);
  console.log(`  PROBE placements in section 0: ${JSON.stringify(probeDoc)}`);
